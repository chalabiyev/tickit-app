...
import {
  Loader2, X, Plus, Minus, ReceiptText, Ticket, CheckCircle2,
  Download, Send, Lock, AlertTriangle, AlertCircle,
} from "lucide-react"
...
// ── Types ──────────────────────────────────────────────────────────────────
export interface Seat { row: number; col: number; tierId: string }
...
export interface EventData {
  ...
  address?: string
  eventDate?: string
  // extra display fields used by events-view
  date?: string
  location?: string
  sold?: number
  total?: number
  ...
  ticketDesign?: { bgColor?: string; elements?: TicketElement[] }
}
...
function findTier(tiers: Tier[] | undefined, tierId: string | undefined): Tier | undefined {
  if (!tiers || !tierId) return undefined
  return tiers.find(
    (ti) =>
...
interface SeatMapProps {
  ...
  selectedAdminSeats: string[]
  onToggleSeat: (key: string) => void
  onToggleAdminSeat: (key: string) => void
  bounds: Bounds
  rowLabels: Record<number, string>
}

function SeatMap({ event, selectedSeats, selectedAdminSeats, onToggleSeat, onToggleAdminSeat, bounds, rowLabels }: SeatMapProps) {
  const { locale } = useLocale()
  const config = event.seatMapConfig ?? {}
  const stages: Stage[] = config.stages ?? [{
    id: "stage-default",
    x: config.stageRect?.x ?? -100, y: config.stageRect?.y ?? -150,
    w: config.stageRect?.w ?? 200,  h: config.stageRect?.h ?? 50,
  }]
  ...
  const activeSelection = isUnblockMode ? selectedAdminSeats : selectedSeats

  const selectedSeatsData: SeatData[] = useMemo(() => activeSelection.map((key) => {
    const [r, c] = key.split("_").map(Number)
    const seat   = event.seats?.find((s) => s.row === r && s.col === c)
    const tier   = findTier(event.tiers, seat?.tierId)
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-5 h-full min-h-[500px]">
        ...
        >
          <div className="relative pointer-events-auto" style={{ width: actualGridWidth, height: actualGridHeight }}>
            ...
            {event.seats?.map((seat) => {
              ...
              const isSelAdmin     = selectedAdminSeats.includes(seatKey)
              const isAdminBlock   = adminSeats.includes(seatKey)
              const isActuallySold = soldSeats.includes(seatKey) && !isAdminBlock

              let seatBg    = tier?.color ?? "#334155"
              let opacity   = 1
              let clickable = true

              if (isActuallySold)               { seatBg = "#333333"; opacity = 0.4; clickable = false }
              else if (isSelected || isSelAdmin) { seatBg = "#ffffff" }
              ...
              return (
                <button
                  ...
                  key={seatKey}
                  onClick={() => {
                    if (!clickable || !tier) return
                    isAdminBlock ? onToggleAdminSeat(seatKey) : onToggleSeat(seatKey)
                  }}
                  disabled={!clickable || !tier}
                  style={{
                    position: "absolute",
                    ...
                    color: isSelected || isSelAdmin ? "#000000" : "#ffffff",
                    opacity,
                  }}
                  className={cn(
                    ...
                    isSelAdmin  && "ring-4 ring-destructive z-10",
                    !isSelected && !isSelAdmin && clickable && "hover:brightness-110",
                    !clickable  && "cursor-not-allowed"
                  )}
                >
                  {isActuallySold ? (
                    <X className="w-3 h-3 opacity-50" />
                  ) : isAdminBlock && !isSelAdmin ? (
      ...
      <div className="flex-1 lg:max-w-[320px] flex flex-col bg-card border border-border/40 rounded-[2rem] overflow-hidden shadow-xl" style={{ isolation: "isolate" }}>
        <div className={cn("p-4 border-b flex items-center gap-2 transition-colors", isUnblockMode ? "bg-destructive/10 border-destructive/20" : "bg-secondary/10")}>
          ...
          <Badge variant={isUnblockMode ? "destructive" : "default"} className="ml-auto font-bold px-2 h-5 text-[10px]">{activeSelection.length}</Badge>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2 min-h-[300px]">
          {selectedSeatsData.length === 0 ? (
            ...
            >
              <button onClick={() => isUnblockMode ? onToggleAdminSeat(s.key) : onToggleSeat(s.key)} className="absolute top-3 right-3 text-muted-foreground hover:text-destructive transition-colors">
                <X className="h-4 w-4" />
              </button>
              ...
              <div className="flex justify-between items-end">
                ...
                {isUnblockMode
                  ? <span className="font-black text-destructive text-xs uppercase">{t(locale, "cancel") || "Ləğv"}</span>
...
// ── AdminBookingModal ──────────────────────────────────────────────────────
export function AdminBookingModal({ event, onClose, onSuccess }: AdminBookingModalProps) {
  const { locale } = useLocale()
  ...
  const toggleSeat = (key: string) => {
    ...
    setSelectedSeats((prev) => prev.includes(key) ? prev.filter((k) => k !== key) : [...prev, key])
  }
  const toggleAdminSeat = (key: string) => {
    setSelectedSeats([])
    setSelectedAdminSeats((prev) => prev.includes(key) ? prev.filter((k) => k !== key) : [...prev, key])
  }

  const handleIncrementTier = (id: string) => setSelectedTiers((prev) => ({ ...prev, [id]: (prev[id] ?? 0) + 1 }))
  const handleDecrementTier = (id: string) => setSelectedTiers((prev) => {
    const next = (prev[id] ?? 0) - 1
    if (next <= 0) { const s = { ...prev }; delete s[id]; return s }
    return { ...prev, [id]: next }
  })

  const selectedCount = isReservedMap
  ...
  const executeUnblockMultiple = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/v1/orders/admin-unbook`, {
        method: "POST",
  ...
  const handleAdminBooking = async () => {
    ...
    try {
      ...
      if (isReservedMap) {
        for (const seatKey of selectedSeats) {
          ...
          ticketsToGenerate.push({
            seatKey, tier,
            seatLabel: `${t(locale, "row") || "Sıra"} ${rowLabels[r] ?? r}, ${t(locale, "seat") || "Yer"} ${c - (bounds?.minC ?? 0) + 1}`,
          })
        }
      } else {
        Object.entries(selectedTiers).forEach(([tierId, qty]) => {
          const tier = event.tiers?.find((ti) => String(ti._safeId) === tierId || String(ti.id) === tierId)
          for (let i = 0; i < qty; i++) {
      ...
      const payload = {
        ...
        customerEmail: bookingType === "block" ? "admin@eticksystem.com" : form.email,
        customerPhone: "-",
      ...
      const res = await fetch(`${API_BASE}/api/v1/orders/admin-book`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      ...
      if (bookingType === "invite" && inviteMethod === "download") {
        const backendTickets = (orderData.tickets ?? []) as { seatId: string; qrCode: string }[]
        ...
        for (const tData of ticketsToGenerate) {
          ...
          const qrData     = matched ? matched.qrCode : `${event.shortLink ?? ""}-${tData.seatKey}`
          ...
          const elements: TicketElement[] = design.elements?.length ? design.elements : [
            { id: "d1", type: "text", x: 24, y: 100, content: "{{Event_Name}}", color: "#ffffff", fontSize: 32, fontWeight: "bold" },
            { id: "d2", type: "text", x: 24, y: 150, content: "{{Event_Date}}", color: "#a1a1aa", fontSize: 14 },
            { id: "d3", type: "text", x: 24, y: 220, content: "{{Guest_Name}}", color: "#ffffff", fontSize: 24, fontWeight: "bold" },
            { id: "d4", type: "text", x: 24, y: 260, content: "{{Ticket_Type}} • {{Seat_Info}}", color: "#3b82f6", fontSize: 16, fontWeight: "bold" },
            { id: "d5", type: "qr",   x: 125, y: 500, content: "", color: "#ffffff", fontSize: 150 },
          ]

          for (const el of elements) {
            if (el.type === "text") {
              const text = (el.content ?? "")
                .replace(/{{Event_Name}}/gi,  event.title ?? event.name ?? "")
                .replace(/{{Event_Date}}/gi,  event.eventDate ?? "")
                .replace(/{{Location}}/gi,    event.isPhysical ? (event.venueName ?? event.address ?? "") : "Online Event")
                .replace(/{{Guest_Name}}/gi,  `${form.firstName} ${form.lastName}`)
              ...
              const size  = el.fontSize ?? 150
              const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(qrData)}`
              const qrImg = Object.assign(new Image(), { crossOrigin: "anonymous", src: qrUrl })
              await new Promise<void>((resolve) => {
                qrImg.onload  = () => { ctx.drawImage(qrImg, el.x, el.y, size, size); resolve() }
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[120] flex items-center justify-center bg-background/90 p-4 pt-24 animate-in fade-in"
        ...
        <Card className={cn(
          ...
          <div className="p-6 border-b flex justify-between items-center bg-secondary/10 shrink-0">
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-bold text-muted-foreground uppercase">{event.name ?? event.title}</span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 relative">
            {errorKey && (
              <div className="flex items-center gap-2 mb-4 px-4 py-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm font-semibold animate-in fade-in">
                <AlertCircle className="h-4 w-4 shrink-0" />
                <span>{t(locale, errorKey) || t(locale, "errUnknown") || "Xəta baş verdi"}</span>
              </div>
            )}
              ...
              <div className="flex flex-col gap-6 h-full">
                {isReservedMap && bounds ? (
                  <div className="flex-1 min-h-[400px]">
                    <SeatMap
                      event={event} selectedSeats={selectedSeats} selectedAdminSeats={selectedAdminSeats}
                      onToggleSeat={toggleSeat} onToggleAdminSeat={toggleAdminSeat}
                      bounds={bounds} rowLabels={rowLabels}
                    />
                  </div>
                ) : (
                  <div className="flex flex-col gap-4 max-w-lg mx-auto w-full pt-10 pb-20">
                    ...
                    {event.tiers?.map((tier, index) => {
                      const tierId = String(tier.id ?? tier._safeId ?? `tier-${index}`)
                      return (
                        <div key={tier.tierId ?? tier.id ?? tier._safeId ?? `tier-${index}`} className="flex items-center justify-between p-5 bg-card border border-border/60 rounded-2xl shadow-sm">
                          <div className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded-full" style={{ backgroundColor: tier.color ?? "#ccc" }} />
                            <h3 className="font-bold">{tier.name}</h3>
                          </div>
                          <div className="flex items-center gap-4 bg-secondary/20 p-2 rounded-xl">
                            <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleDecrementTier(tierId)} disabled={!selectedTiers[tierId]}><Minus className="w-4 h-4" /></Button>
                            <span className="font-bold w-6 text-center">{selectedTiers[tierId] ?? 0}</span>
                            <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleIncrementTier(tierId)}><Plus className="w-4 h-4" /></Button>
                          </div>
                        </div>
                      )
                    })}
                ...
                <div className="flex items-center justify-between bg-secondary/20 p-4 rounded-2xl shrink-0 mt-auto">
                  {selectedAdminSeats.length > 0 ? (
                    <>
                      ...
                      <Button variant="destructive" className="rounded-xl px-8 font-bold" onClick={() => setShowConfirmModal(true)} disabled={loading}>
                    ...
                    <>
                      ...
                      <Button disabled={selectedCount === 0} className="rounded-xl px-8 font-bold" onClick={() => setStep(2)}>
            ...
            {step === 2 && (
              <div className="flex flex-col gap-5 animate-in slide-in-from-right-4">
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="space-y-2">
                      <Label className="text-[10px] font-bold uppercase opacity-60 ml-1">
                        ...
                        {inviteMethod === "email" ? <span className="text-destructive">*</span> : `(${t(locale, "optional") || "İstəyə bağlı"})`}
                      </Label>
                    ...
                    {inviteMethod === "email" && (
                      <div className="space-y-2">
                        ...
                        <textarea
                          className="flex min-h-[80px] w-full rounded-xl border-none bg-secondary/10 px-3 py-2 text-sm shadow-inner placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary resize-none"
                ...
                <div className="flex gap-3 mt-2">
                  ...
                  <Button className="flex-[2] h-14 rounded-xl font-black uppercase tracking-widest shadow-lg" onClick={handleAdminBooking} disabled={loading || !isFormValid}>
                    {loading
                      ? <Loader2 className="animate-spin" />
                      : bookingType === "invite" && inviteMethod === "email"
                        ? (t(locale, "confirmAndSend") || "Təsdiqlə və Göndər")
                        : (t(locale, "confirm") || "Təsdiqlə")
                    }
                  </Button>
                </div>
              </div>
                ...
                <p className="text-muted-foreground font-medium mt-2 max-w-sm">
                  {bookingType === "invite"
                    ? inviteMethod === "download"
                      ? (t(locale, "pdfDownloaded")    || "Dəvətnamələr PDF formatında kompüterinizə yükləndi.")
                      : (t(locale, "emailSentSuccess") || "Dəvətnamə qonağın e-poçt ünvanına uğurla göndərildi!")
                    : (t(locale, "seatsBlockedSuccess") || "Seçilmiş biletlər satışdan uğurla çıxarıldı.")}
                </p>
                <Button size="lg" className="mt-10 px-12 rounded-xl font-black h-12 shadow-lg" onClick={() => { onSuccess(); onClose() }}>
                  {t(locale, "finish") || "Bitir"}
                </Button>
              </div>
            )}
          </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/60 animate-in fade-in" style={{ WebkitBackdropFilter: "blur(8px)", backdropFilter: "blur(8px)" }}>
          <Card className="max-w-sm w-full p-6 flex flex-col items-center text-center shadow-2xl border-border/50 rounded-3xl mx-4 animate-in zoom-in-95" style={{ isolation: "isolate" }}>
            <div className="w-16 h-16 bg-destructive/10 rounded-full flex items-center justify-center mb-4">
              <AlertTriangle className="w-8 h-8 text-destructive" />
            </div>
            <h3 className="text-xl font-bold mb-2">{t(locale, "attention") || "Diqqət!"}</h3>
            <p className="text-muted-foreground text-sm mb-6">{t(locale, "confirmUnblockMultiple") || "Seçilmiş yerlərin bronunu ləğv etmək istədiyinizə əminsiniz?"}</p>
            <div className="flex gap-3 w-full">
              <Button variant="outline" className="flex-1 rounded-xl font-bold" onClick={() => setShowConfirmModal(false)} disabled={loading}>{t(locale, "cancel") || "İmtina"}</Button>
