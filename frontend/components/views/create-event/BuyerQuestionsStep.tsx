      ...
      <div className="flex items-center justify-between bg-primary/5 p-6 rounded-2xl border border-primary/10">
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-sm text-muted-foreground">
            {t(locale, "buyerQuestionsDesc") ||
              "Ask attendees for additional information (e.g., Job Title, Company) during checkout."}
          </p>
        </div>
      </div>
        ...
        {questions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed rounded-2xl bg-secondary/10 border-border">
            ...
            <p className="text-sm text-muted-foreground max-w-sm mb-4">
              {t(locale, "defaultQuestionsInfo") ||
                "We will only ask for Name, Email, and Phone number by default."}
            </p>
            <Button variant="outline" onClick={addQuestion} className="gap-2">
              <Plus className="h-4 w-4" />
              {t(locale, "addCustomQuestion") || "Add Custom Question"}
            </Button>
          </div>
        ) : (
          <>
            {questions.map((q) => (
              <div
                key={q.id}
                className="flex flex-col sm:flex-row gap-4 p-5 rounded-xl border bg-card shadow-sm items-start sm:items-center relative group"
              >
                <div className="flex-1 w-full flex flex-col gap-2">
                  <Label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
                    {t(locale, "questionLabel") || "Question Label"}{" "}
                    <span className="text-destructive">*</span>
                  </Label>
                  <Input
                    placeholder={
                      t(locale, "questionLabelPlaceholder") || "e.g., Job Title, Company Name"
                    }
                    value={q.label}
            ...
            >
              ...
              {t(locale, "addAnotherQuestion") || "Add Another Question"}
            </Button>
          </>
        )}
      </div>
