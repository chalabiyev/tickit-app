...
import {
  ...
  CheckCircle2, AlertCircle, Video, Ticket, Search, X, Loader2
} from "lucide-react"
...
// ВСТАВЬ ЭТОТ КОД ДО export function CreateEventWizard(...)
...
export function CreateEventWizard({ onBack }: CreateEventWizardProps) {
  ...
  const handleCopyLink = async () => {
    try {
      ...
      showToast(t(locale, "linkCopied") || "Link copied to clipboard!", "success");
    } catch (err) {
      showToast(t(locale, "linkCopyFailed") || "Failed to copy link", "error");
    }
  };
  ...
  // СОХРАНЯЕМ КООРДИНАТЫ ОБРЕЗКИ
  const onCropComplete = (croppedArea: any, croppedAreaPixels: any) => {
    setCroppedAreaPixels(croppedAreaPixels)
  }

  // ОБРЕЗАЕМ И ОТПРАВЛЯЕМ НА БЭКЕНД
  const handleCropSave = async () => {
    if (!imageSrc || !croppedAreaPixels) return
    setIsUploading(true)
    setIsCropperOpen(false) // Закрываем модалку
    try {
  ...
  const submitEvent = async () => {
    ...
    const payload = {
      ...
      tiers: tiers.map(t => ({ 
        tierId: t.id.toString(), id: t.id.toString(), name: t.name, price: t.price, quantity: t.quantity, color: t.color 
      })),
          ...
          <div className="p-5 sm:p-0">
              ...
              <div className="flex flex-col gap-10 animate-in fade-in duration-300">
                <div className="flex flex-col gap-6">
                  <div className="flex flex-col gap-2.5">
                    ...
                    <Input placeholder={t(locale, "eventTitlePlaceholder") || "Məsələn: Rammstein Baku Concert"} value={eventTitle} onChange={(e) => setEventTitle(e.target.value)} className="max-w-2xl h-11 text-base" />
                  </div>
                  ...
                  <div className="flex flex-col gap-5 sm:flex-row">
                    <div className="flex flex-col gap-2.5 flex-1 max-w-[250px]">
                      ...
                      <Select value={category} onValueChange={setCategory}>
                        <SelectTrigger className="h-11"><SelectValue placeholder={t(locale, "selectCategory") || "Select Category"} /></SelectTrigger>
                ...
                <div className="flex flex-col gap-8 lg:flex-row lg:gap-12">
                    ...
                    {isPhysical ? (
                      <div className="flex flex-col gap-5 mt-2 animate-in fade-in slide-in-from-top-2">
                        <div className="flex flex-col gap-2.5">
                          ...
                          <Input className="h-11 shadow-sm text-base" placeholder={t(locale, "venueExampleAz") || "Məsələn: Crystal Hall"} value={venueName} onChange={(e) => setVenueName(e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2.5">
                          <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                      ...
                      <div className="flex flex-col gap-5 mt-2 animate-in fade-in slide-in-from-top-2 p-5 bg-secondary/10 border border-primary/20 rounded-2xl">
                        ...
                        <div className="flex flex-col gap-2.5">
                          ...
                          <Input className="h-11 shadow-sm text-base bg-background" placeholder={t(locale, "streamUrlPlaceholder") || "https://zoom.us/j/123..."} value={streamUrl} onChange={(e) => setStreamUrl(e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2.5">
                          <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                            {t(locale, "streamPasswordLabel") || "Password / Instructions (Optional)"}
                          </Label>
                          <Input className="h-11 shadow-sm text-base bg-background" placeholder={t(locale, "streamPasswordPlaceholder") || "Passcode: 123456"} value={streamPassword} onChange={(e) => setStreamPassword(e.target.value)} />
                        </div>
                  ...
                  <div className="flex flex-col gap-5 flex-1">
                    <Label className="text-base font-bold">{t(locale, "privacySettingsTitle") || "Privacy Settings"}</Label>
                    ...
                    <div className="flex flex-col gap-2.5 mt-4">
                      <Label className="text-sm font-semibold flex items-center gap-2 text-foreground">
                        <Ticket className="w-4 h-4 text-primary"/> {t(locale, "maxTicketsPerOrder") || "Bir sifarişdə maksimum bilet"}
                      </Label>
                      <p className="text-xs text-muted-foreground mb-1">
                        {t(locale, "maxTicketsDesc") || "Bir istifadəçinin eyni anda ala biləcəyi maksimum bilet sayı (qara bazara qarşı qorunma)."}
                      </p>
                      <Input 
                        type="number" 
                        min={1} 
                        ...
                        onChange={(e) => setMaxTicketsPerOrder(Number(e.target.value))} 
                        className="h-11 bg-background max-w-sm font-bold"
                      />
                    </div>

                    <div className="flex flex-col gap-3 mt-1">
                      <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t(locale, "eventPosterTitle") || "Event Poster"} <span className="text-destructive">*</span></Label>
                      ...
                      <div onClick={() => fileInputRef.current?.click()} className={cn("flex h-36 items-center justify-center rounded-xl border-2 border-dashed transition-all cursor-pointer overflow-hidden relative group", coverImageUrl ? "border-primary/50" : "border-border/80 bg-secondary/30 hover:border-primary/50 hover:bg-secondary/50", isUploading && "opacity-50 cursor-wait")}>
                        {isUploading ? (
                          ...
                          <div className="flex flex-col items-center gap-3 text-muted-foreground"><div className="p-3 bg-background rounded-full shadow-sm border"><Upload className="h-5 w-5 text-foreground" /></div><span className="text-sm font-semibold">{t(locale, "clickOrDrag") || "Upload Cover Image"}</span><span className="text-[10px] uppercase tracking-widest opacity-60">{t(locale, "recommendedSize") || "1920x1080px"}</span></div>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            {currentStep === 6 && (
              <div className="flex flex-col gap-10 lg:flex-row lg:gap-14 animate-in fade-in duration-300">
                <div className="flex-1 flex flex-col gap-5"><h3 className="text-lg font-bold text-foreground">{t(locale, "platformFeeTitle") || "Platform Fee"}</h3><Card className="border-none bg-primary text-primary-foreground relative overflow-hidden shadow-xl"><div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-20 -mt-20 blur-3xl pointer-events-none" /><CardContent className="flex flex-col gap-6 p-8 relative z-10"><div className="flex flex-col gap-1"><span className="text-sm font-semibold text-primary-foreground/80 uppercase tracking-widest">{t(locale, "feeForSeats")?.replace("{cap}", derivedCapacity.toString()) || `Fee for ${derivedCapacity} seats`}</span><span className="text-6xl font-black mt-2">{platformFee === 0 ? (t(locale, "freeText") || "Free") : `${platformFee} AZN`}</span></div><Separator className="bg-primary-foreground/20 my-2" /><div className="flex flex-col gap-3 text-sm font-medium text-primary-foreground/90"><span className="uppercase tracking-wider text-xs font-bold text-primary-foreground/70 mb-1">{t(locale, "pricingRules") || "eticksystem Pricing Rules"}</span><div className="flex justify-between border-b border-primary-foreground/10 pb-2"><span>0 - 10 {t(locale, "seats") || "seats"}</span> <span>0 AZN</span></div><div className="flex justify-between border-b border-primary-foreground/10 pb-2"><span>11 - 50 {t(locale, "seats") || "seats"}</span> <span>5 AZN</span></div><div className="flex justify-between border-b border-primary-foreground/10 pb-2"><span>51 - 100 {t(locale, "seats") || "seats"}</span> <span>10 AZN</span></div><div className="flex justify-between"><span>101 - 200 {t(locale, "seats") || "seats"}</span> <span>15 AZN</span></div></div></CardContent></Card></div>
                <div className="flex-1 flex flex-col gap-5"><h3 className="text-lg font-bold text-foreground">{t(locale, "paymentMethodTitle") || "Payment Method"}</h3><Card className="border-border/60 shadow-md"><CardContent className="p-8 flex flex-col gap-6">{platformFee === 0 ? (<div className="flex flex-col items-center justify-center py-10 text-center h-full"><div className="h-16 w-16 rounded-full bg-green-500/10 flex items-center justify-center mb-5"><Check className="h-8 w-8 text-green-500" /></div><p className="text-xl font-bold text-foreground mb-2">{t(locale, "noPaymentRequired") || "No payment required"}</p><p className="text-sm text-muted-foreground max-w-[250px] leading-relaxed">{t(locale, "noPaymentDesc") || "Your event capacity is 10 seats or under, so publishing is completely free!"}</p></div>) : (<div className="flex flex-col gap-5"><div className="flex flex-col gap-2.5"><Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{t(locale, "cardholderName") || "Cardholder Name"}</Label><Input placeholder={t(locale, "johnDoe") || "JOHN DOE"} className="h-12 uppercase font-medium" /></div><div className="flex flex-col gap-2.5"><Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{t(locale, "cardNumber") || "Card Number"}</Label><div className="relative"><CreditCard className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" /><Input placeholder="0000 0000 0000 0000" className="pl-12 h-12 font-mono text-base" /></div></div><div className="flex gap-5"><div className="flex flex-col gap-2.5 flex-1"><Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{t(locale, "expiry") || "Expiry"}</Label><Input placeholder={t(locale, "mm_yy") || "MM/YY"} className="h-12 font-mono text-center text-base" /></div><div className="flex flex-col gap-2.5 flex-1"><Label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{t(locale, "cvc") || "CVC"}</Label><Input placeholder="123" type="password" maxLength={3} className="h-12 font-mono text-center text-base tracking-widest" /></div></div></div>)}</CardContent></Card></div>
              </div>
            )}
          ...
          <div className="bg-background rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl flex flex-col m-4 border border-border/50">
            ...
            <div className="relative w-full h-[50vh] sm:h-[60vh] bg-black">
              <Cropper
                ...
                crop={crop}
                zoom={zoom}
                aspect={16 / 9} // ЖЕСТКАЯ ФИКСАЦИЯ 1920x1080
...
// 2. ВСТРАИВАЕМ КОМПОНЕНТ ПОИСКА ЛОКАЦИИ (Также обновили плейсхолдеры)
  ...
  return (
    <div className="flex flex-col gap-4 w-full relative">
      <div className="relative z-50">
        ...
        <Input placeholder={t(locale, "addressSearchPlaceholder") || "Məsələn: Nizami küçəsi 50, Bakı"} className="pl-10 h-11 text-base shadow-sm bg-secondary/20" value={address} onChange={(e) => handleSearch(e.target.value)} />
